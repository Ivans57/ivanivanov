...
class CreateFunctionGetParentIds extends Migration
{
    ...
    public function up()
    {
        ...
        RETURNS JSON
            BEGIN		
		CASE _table_name
                    WHEN "en_albums" THEN SET @parents := (SELECT parents from en_albums_ids_nesting_levels_parents_children WHERE items_id = _parent_id);
                    WHEN "ru_albums" THEN SET @parents := (SELECT parents from ru_albums_ids_nesting_levels_parents_children WHERE items_id = _parent_id);
                    WHEN "en_folders" THEN SET @parents := (SELECT parents from en_folders_ids_nesting_levels_parents_children WHERE items_id = _parent_id);
                    WHEN "ru_folders" THEN SET @parents := (SELECT parents from ru_folders_ids_nesting_levels_parents_children WHERE items_id = _parent_id);
		END CASE;
		...
		RETURN @parents;
            END;
        ';
        $GetParentIDs = str_replace("*","'", $GetParentIDsCode);
        DB::unprepared($GetParentIDs);
    }
