...
class CreateTriggerChildrenArrayChangeBeforeEnAlbumsDelete extends Migration
{
    ...
    public function up()
    {
		...
		SET @parents := (SELECT parents FROM en_albums_data WHERE items_id = OLD.id);
		
		SET @items_ids_to_remove := (SELECT children FROM en_albums_data WHERE items_id = OLD.id);
		...
		WHILE (_counter < JSON_LENGTH(@parents)) DO
                    ...
                    UPDATE en_albums_data SET children = RemoveItemFromJSON(@one_parent_id, @items_ids_to_remove, "en_albums_data", "children") 
