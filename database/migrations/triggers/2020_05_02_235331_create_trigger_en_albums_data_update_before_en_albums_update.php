...
class CreateTriggerEnAlbumsDataUpdateBeforeEnAlbumsUpdate extends Migration
{
    ...
    public function up()
    {
		...
		IF ((NEW.included_in_album_with_id <=> OLD.included_in_album_with_id) = 0) THEN
                    ...
                    SET @items_children := (SELECT children FROM en_albums_data WHERE items_id = OLD.id);

                    SET @items_parents_to_remove := (SELECT parents FROM en_albums_data WHERE items_id = OLD.id);
                    SET @new_parents_without_prev := (SELECT parents from en_albums_data where items_id = NEW.included_in_album_with_id);
                    ...
                    IF (@items_children IS NULL) THEN
                        ...
                        ELSE				
                            SET @new_nesting_level := (SELECT nesting_level from en_albums_data where items_id = NEW.included_in_album_with_id) + 1;
                            SET @new_parents := (SELECT parents from en_albums_data where items_id = NEW.included_in_album_with_id);
                        ...
                        UPDATE en_albums_data SET nesting_level = @new_nesting_level, parents = @new_parents WHERE items_id = OLD.id;
                    ELSE
                        #Here we need to include being moved items children ids and its id.
                        SET @items_children := JSON_ARRAY_INSERT(@items_children, "$[0]", CONVERT(OLD.id, CHAR));
                        SET @old_nesting_level := (SELECT nesting_level from en_albums_data where items_id = OLD.id);
                        ...
                        WHILE (_counter < JSON_LENGTH(@items_children)) DO
                            ...
                            ELSE				
                                ...
                                SET @new_nesting_level := (SELECT nesting_level from en_albums_data WHERE items_id = NEW.included_in_album_with_id) + 1 + @nesting_level_relating_to_root_parent;				
                            END IF;
                            ...
                            ELSE
                                SET @parent_ids_without_old_parents := RemoveItemFromJSON(@current_items_id, @items_parents_to_remove, "en_albums_data", "parents");
                            END IF;
                            ...
                            UPDATE en_albums_data SET nesting_level = @new_nesting_level, parents = @new_parents WHERE items_id = @current_items_id;

                            SET _counter := _counter + 1;

                        END WHILE;			
                    END IF; 
                    CALL UpdateChildrenInEnAlbumsData(@items_children, @items_parents_to_remove, @new_parents_without_prev, "en_albums_data", "children");
		END IF;
            END
        ');
    }
