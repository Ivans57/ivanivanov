...
class CreateTriggerNestingLevelCheckBeforeRuAlbumsInsert extends Migration
{
    ...
    {
        DB::unprepared('
        CREATE TRIGGER nesting_level_check_before_ru_albums_insert BEFORE INSERT ON `ru_albums` FOR EACH ROW
            BEGIN
		IF (NEW.included_in_album_with_id IS NOT NULL) THEN 					
                    IF ((SELECT nesting_level FROM ru_albums_data WHERE items_id = NEW.included_in_album_with_id) > 6) 
