...
class CreateStoredProcedureUpdateChildrenInEnAlbumsIdsNestingLevelsParentsChildren extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    {
        DB::unprepared('
        CREATE PROCEDURE UpdateChildrenInEnAlbumsIdsNestingLevelsParentsChildren(IN _children JSON, IN _old_parent_ids JSON, IN _new_parent_ids JSON, IN _table_name VARCHAR(45), IN _field_name VARCHAR(45))
            BEGIN
		...
		IF (_old_parent_ids IS NOT NULL) THEN
			...
			CASE _table_name
				...
				UPDATE en_albums_ids_nesting_levels_parents_children SET children = @new_children WHERE items_id = @current_parents_id;
                            WHEN "ru_albums_ids_nesting_levels_parents_children" THEN 
				UPDATE ru_albums_ids_nesting_levels_parents_children SET children = @new_children WHERE items_id = @current_parents_id;
                            WHEN "en_folders_ids_nesting_levels_parents_children" THEN 
				UPDATE en_folders_ids_nesting_levels_parents_children SET children = @new_children WHERE items_id = @current_parents_id;
                            WHEN "ru_folders_ids_nesting_levels_parents_children" THEN 
				UPDATE ru_folders_ids_nesting_levels_parents_children SET children = @new_children WHERE items_id = @current_parents_id;
			END CASE;
    ...
    {
        DB::unprepared('DROP PROCEDURE UpdateChildrenInEnAlbumsIdsNestingLevelsParentsChildren');
    }
}
