...
class AlbumCreateOrEditRepository {
    ...
    private function get_full_directory_path($directory_id, $directory_path) {//+-
        ...
        $directory = \App\Album::select('album_name', 'included_in_album_with_id')
                ->where('id', $directory_id)->firstOrFail();
        $directory_full_path = substr_replace($directory_path, ' / '.$directory->album_name, 0, 0);
        if ($directory->included_in_album_with_id != 0){
            $directory_full_path = $this->get_full_directory_path($directory->included_in_album_with_id, $directory_full_path);
        }
    ...
    private function get_directory_children_array($directory_to_exclude_keyword) {//-+
        if($directory_to_exclude_keyword) {
        
            //We will do two request to avoid localization check.
            $directory_id = \App\Album::select('id')->where('keyword', $directory_to_exclude_keyword)->firstOrFail();           
            $items_children_from_query = \App\AlbumData::select('children')
                    ->where('items_id', $directory_id->id)->firstOrFail();
    ...
    private function get_closed_parent_list($localization, $page, $records_to_show, $parent_node_id, $keyword_of_directory_to_exclude) {//-+
        ...
        if ($keyword_of_directory_to_exclude) {
            $id_of_directory_to_exclude = \App\Album::select('id')->where('keyword', $keyword_of_directory_to_exclude)->firstOrFail();
        }
    ...
    private function make_parent_object_array_for_parent_list($parent_list_from_query, $max_acceptable_nest_level, 
        ...
        foreach ($parent_list_from_query as $directory) {
            ...
            $parent_data_array->DirectoryId = $directory->id;
            $parent_data_array->DirectoryName = $directory->album_name;
            ...
            if ($directory->children === null || $directory->nesting_level === ($max_acceptable_nest_level - 1)) {
                $parent_data_array->HasChildren = false;
            } else if ($id_of_directory_to_exclude) {
                $parent_data_array->HasChildren = $this->check_for_children($directory->id, $id_of_directory_to_exclude->id);
            }
            ...
            if ($parent_id) {
                ...
                $parent_data = $this->check_if_open_or_in_focus($directory->id, $parent_id, $iteration);
    ...
    private function check_for_children($directory_id, $id_of_directory_to_exclude) {//-+
        ...
        $items_direct_children = \App\Album::select('id')
                        ->where('included_in_album_with_id', '=', $directory_id)
    ...
    private function get_max_acceptable_nest_level($directory_to_exclude_keyword = NULL) {//-+
               
        $max_acceptable_nest_level = 7;
        
        if ($directory_to_exclude_keyword != NULL) {
            $items_id = \App\Album::where('keyword', $directory_to_exclude_keyword)->select('id')->firstOrFail();
    ...
    private function get_children_max_nest_level($items_children, $max_acceptable_nest_level) {//-+
        ...
        $children_nest_levels = \App\AlbumData::whereIn('items_id', array_map('intval', $items_children))
